...
// path. Sessions there are best-effort (lost on cold starts); the client
// auto-recreates a stale session, so the app degrades gracefully.
const SESSION_DIR = process.env.VERCEL
  ? path.join('/tmp', 'concept-forge-sessions')
...
function addChat(session, role, text, extra) {
  if (!text) return;
  session.chat.push({ role, text, at: new Date().toISOString(), ...(extra || {}) });
