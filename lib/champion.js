...
function unescapeLiterals(v) {
  if (typeof v === 'string') {
    return v
      .replace(/\\u([0-9a-fA-F]{4})/g, (_, h) => String.fromCharCode(parseInt(h, 16)))
      .replace(/\\n/g, '\n').replace(/\\t/g, ' ').replace(/\\"/g, '"');
  }
  if (Array.isArray(v)) return v.map(unescapeLiterals);
  if (v && typeof v === 'object') return Object.fromEntries(Object.entries(v).map(([k, x]) => [k, unescapeLiterals(x)]));
  return v;
}
...
async function polishChampion({ deck, card }) {
  ...
  return unescapeLiterals(extractToolInput(response, 'emit_champion'));
}
